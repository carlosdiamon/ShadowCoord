...
public class OffsettedColumn extends Column {
    ...
    private final CoordinateOffset offset;
    private final User user;

    private static final BaseChunk[] emptyChunkList = {};

    public OffsettedColumn(Column column, CoordinateOffset offset, User user) {
    ...
    public TileEntity[] getTileEntities() {
        ...
        if (user.getClientVersion().isOlderThan(ClientVersion.V_1_18)) {
            ...
            for (TileEntity entity : entities) {
                entity.setX(entity.getX() - offset.coordX());
                entity.setZ(entity.getZ() - offset.coordZ());
            }
            return entities;
        } else {
